...
try {
    ...
    process.exit(1); // Exit if keys can't be set up
}
    ...
    if (err) {
        ...
        db.serialize(() => {
            ...
            db.run(`CREATE TABLE IF NOT EXISTS sessions (
                ...
                // Once all tables are guaranteed to exist, set up the rest of the app.
                initializeApp();
            });
        });
    }
...
// --- Helper Functions --- (These can stay outside the main init function)
...
// This function contains all the server setup that depends on the database being ready.
function initializeApp() {
        ...
        db.get("SELECT * FROM users WHERE username = ?", [username], (err, user) => {
            ...
            bcrypt.compare(password, user.password, (err, result) => {
                if (result) {
                    ...
                    res.json({ success: true, user: { username: user.username, isAdmin: user.isAdmin === 1 } });
                } else {
        ...
        if (id) {
            ...
            if (req.file) { 
                if (sourceToUpdate.type === 'file' && fs.existsSync(sourceToUpdate.path)) {
                    try { fs.unlinkSync(sourceToUpdate.path); } catch (e) { console.error("Could not delete old source file:", e); }
                }
            ...
            const newSource = { id: `src-${Date.now()}`, name, type: req.file ? 'file' : 'url', path: req.file ? req.file.path : url, isActive: isActive === 'true', lastUpdated: new Date().toISOString(), status: 'Pending', statusMessage: 'Source added. Process to load data.' };
            if (newSource.type === 'url' && !newSource.path) return res.status(400).json({ error: 'URL is required for URL-type source.' });
            if (req.file) {
                const extension = sourceType === 'm3u' ? '.m3u' : '.xml';
    ...
    app.put('/api/sources/:sourceType/:id', requireAuth, (req, res) => {
        ...
        if (source.type === 'url') source.path = newPath ?? source.path;
        source.lastUpdated = new Date().toISOString();
        saveSettings(settings);
    ...
    app.delete('/api/sources/:sourceType/:id', requireAuth, (req, res) => {
        ...
        if (source && source.type === 'file' && fs.existsSync(source.path)) {
            try { fs.unlinkSync(source.path); } catch (e) { console.error(`Could not delete source file: ${source.path}`, e); }
        }
    ...
    app.post('/api/user/settings', requireAuth, (req, res) => {
        ...
        db.run(`INSERT INTO user_settings (user_id, key, value) VALUES (?, ?, ?) ON CONFLICT(user_id, key) DO UPDATE SET value = excluded.value`,
            [userId, key, valueJson], (err) => {
            if (err) {
    ...
    app.get('/api/notifications/vapid-public-key', requireAuth, (req, res) => {
        if (!vapidKeys.publicKey) return res.status(500).json({ error: 'VAPID public key not available on the server.' });
    ...
    app.post('/api/notifications/subscribe', requireAuth, (req, res) => {
        ...
        db.run(`INSERT INTO push_subscriptions (user_id, endpoint, p256dh, auth) VALUES (?, ?, ?, ?) ON CONFLICT(endpoint) DO UPDATE SET user_id=excluded.user_id`,
            [userId, endpoint, p256dh, auth], function(err) {
            if (err) {
    ...
    app.post('/api/notifications/unsubscribe', requireAuth, (req, res) => {
        ...
        if (!endpoint) return res.status(400).json({ error: 'Endpoint is required to unsubscribe.' });
    ...
    app.post('/api/notifications', requireAuth, (req, res) => {
        ...
        db.run(`INSERT INTO notifications (user_id, channelId, channelName, channelLogo, programTitle, programDesc, programStart, programStop, notificationTime, programId)
                ...
                ON CONFLICT(user_id, programId) DO UPDATE SET
                    channelName=excluded.channelName, channelLogo=excluded.channelLogo, programTitle=excluded.programTitle,
                    programDesc=excluded.programDesc, programStart=excluded.programStart, programStop=excluded.programStop,
                    notificationTime=excluded.notificationTime, status='active'`,
    ...
    app.get('/api/notifications', requireAuth, (req, res) => {
        const userId = req.session.userId;
        const pastLimit = 10;
        const responsePayload = { active: [], past: [] };
        db.all(`SELECT id, user_id, channelId, channelName, channelLogo, programTitle, programDesc, programStart, programStop, notificationTime as scheduledTime, programId, status
                FROM notifications WHERE user_id = ? AND status = 'active' ORDER BY notificationTime ASC`,
            [userId], (err, activeRows) => {
                if (err) {
                ...
                db.all(`SELECT id, user_id, channelId, channelName, channelLogo, programTitle, programDesc, programStart, programStop, notificationTime as scheduledTime, programId, status, notifiedAt
                        FROM notifications WHERE user_id = ? AND status != 'active' ORDER BY notifiedAt DESC LIMIT ?`,
                    [userId, pastLimit], (err, pastRows) => {
                        if (err) {
    ...
    app.delete('/api/notifications/:id', requireAuth, (req, res) => {
        const { id } = req.params;
        db.run(`DELETE FROM notifications WHERE id = ? AND user_id = ?`, [id, req.session.userId], function (err) {
            if (err) {
            ...
            if (this.changes === 0) return res.status(404).json({ error: 'Notification not found or unauthorized.' });
            res.json({ success: true });
        });
    });
    ...
    app.get('/stream', requireAuth, (req, res) => {
        ...
        const commandTemplate = profile.command.replace(/{streamUrl}/g, streamUrl).replace(/{userAgent}|{clientUserAgent}/g, userAgent.value);
        const args = (commandTemplate.match(/(?:[^\s"]+|"[^"]*")+/g) || []).map(arg => arg.replace(/^"|"$/g, ''));
        const ffmpeg = spawn('ffmpeg', args);
        res.setHeader('Content-Type', 'video/mp2t');
        ffmpeg.stdout.pipe(res);
        ffmpeg.stderr.on('data', (data) => console.error(`[FFMPEG_ERROR] ${streamUrl}: ${data}`));
