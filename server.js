...
async function initializeServer() {
    ...
    console.log('[Init] Checking/creating necessary directories...');
    ...
    try {
        ...
        console.error('[Push] FATAL: Could not load or generate VAPID keys. Exiting.', error);
        process.exit(1); // Critical error, exit process
    }

    // 3. Database Setup
    console.log('[Init] Connecting to SQLite database and ensuring schema...');
    db = await new Promise((resolve, reject) => {
        const sqliteDb = new sqlite3.Database(DB_PATH, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
            ...
            // Ensure all tables are created before resolving the promise
            sqliteDb.serialize(() => {
                let errorOccurred = false;
                const runAndCheck = (sql, callback) => {
                    ...
                    runAndCheck(`CREATE TABLE IF NOT EXISTS user_settings (
                        ...
                        runAndCheck(`CREATE TABLE IF NOT EXISTS notifications (
                            ...
                            runAndCheck(`CREATE TABLE IF NOT EXISTS push_subscriptions (
                                ...
                                if (!errorOccurred) {
                                    resolve(sqliteDb); // Resolve only after the last table is created successfully
                                }
                            });
                        });
                    });
